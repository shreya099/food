...
echo $c=get_safe_value($_POST['category']);
echo $o=get_safe_value($_POST['order_no']);
echo $id;

if(mysqli_num_rows(mysqli_query($conn,"SELECT * from category where category='$c' && id!='$id'"))>0)
 echo "<script>swal( 'Category already exists','Cateory exists!','error' ).then(function() { window. location = 'category.php'; });;</script>";
 else{
